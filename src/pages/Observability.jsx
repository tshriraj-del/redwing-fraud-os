...
// Production fraud catch rate before vs after the training-serving-skew fix
// (measured on held-out data; operator full-pipeline path).
const CATCH = [
  { stage: 'Before — skew', value: 0.3,  fill: '#ef4444' },
  { stage: 'After — fixed', value: 91,   fill: '#22c55e' },
];
    ...
    <div style={{ height: '100%', overflowY: 'auto', padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ background: 'var(--bg-surface)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 10, padding: '18px 22px', display: 'flex', alignItems: 'center', gap: 22 }}>
        <div style={{ textAlign: 'center', flexShrink: 0 }}>
          <div style={{ fontSize: 30, fontWeight: 800, color: 'var(--green)', fontFamily: 'JetBrains Mono, monospace', lineHeight: 1.1 }}>0.984</div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text)', lineHeight: 1.6 }}>
            In the live scoring path the model caught <b style={{ color: 'var(--red)' }}>under 1% of fraud</b> — it was blind to <b>~24% of its own inputs</b> (10 of 23 features couldn't be reproduced at serving and silently defaulted to zero). No offline metric revealed it.
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        <MetricCard label="Offline AUC" value="0.984" sub="What the dashboard reported" />
        <MetricCard label="Field catch — skew" value="0.3%" sub="Live path, before the fix" accent="var(--red)" />
        <MetricCard label="Field catch — fixed" value="91%" sub="After feature-consistency rebuild" accent="var(--green)" />
        <MetricCard label="Features restored" value="23/23" sub="No silent zero-fill" accent="#818cf8" />
      </div>
